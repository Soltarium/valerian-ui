...
-- ==================== NEW HUB UI (7-LINES FIX & UPDATE) ====================
local Players = game:GetService("Players")
local RunService = game:GetService("RunService")
local Stats = game:GetService("Stats")
local Lighting = game:GetService("Lighting")

-- กำหนดค่าโครงสร้างตัวแปรตามระบบของสคริปต์หลัก
if not L_1_ then L_1_ = {} end
if not L_1_[45] then L_1_[45] = {} end

L_1_[5] = Players.LocalPlayer
local player = L_1_[5]

-- เคลียร์ UI และ Blur ตัวเก่าออกก่อนเพื่อป้องกันการทับซ้อน
if game:GetService("CoreGui"):FindFirstChild("Screengui") then
    game:GetService("CoreGui").Screengui:Destroy()
end
if Lighting:FindFirstChild("MeowHubBlur") then
    Lighting.MeowHubBlur:Destroy()
end

-- ลบวัตถุตามเงื่อนไขของตัวระบบเดิม
if L_1_[43] and L_1_[43]:FindFirstChild("Status_UI") then
    L_1_[43]["Status_UI"]:Destroy()
end

-- สร้างระบบคลังข้อความ UI ใหม่
local Library = {}
...
-- สร้างรูปอวาตาร์ตัวละครผู้เล่น (สไตล์ใหม่)
local Image = Instance.new('ImageLabel')
Image.Name = 'Image'
Image.Parent = ScreenGui
Image.AnchorPoint = Vector2.new(0.5, 0.5)
Image.Position = UDim2.new(0.5, 0, 0.24, 0) -- ขยับขึ้นเล็กน้อยเพื่อให้สมดุลกับ 7 บรรทัด
...
-- ฟังก์ชันดั้งเดิมสร้าง TextLabel ตรงจุดกลางจอ
local function CreateTextLabel(text, position)
    ...
    Label.Size = UDim2.new(0, 600, 0, 30)
...
-- จัดเรียงคีย์บอร์ดข้อความใหม่ทั้งหมด 7 แถว (แก้ไขปัญหารันชื่อแล้วบั๊กไม่ขึ้น)
local textLines = {'NameHub', 'Task1', 'Task2', 'Currencies', 'Melees', 'DebugLine', 'Credit'}
for i, v in pairs(textLines) do
    Library[v] = CreateTextLabel("...", UDim2.new(0.5, 0, 0.30 + (.045 * i), 0))
end

-- สร้างฉากหลัง Blur ตามข้อกำหนดใหม่
local BlurEffect = Instance.new("BlurEffect")
BlurEffect.Name = "MeowHubBlur"
BlurEffect.Parent = Lighting
BlurEffect.Size = 12

-- ฟังก์ชันกำหนดอัปเดตตัวหนังสือ
...
-- ตั้งค่าบรรทัดหัวข้อสคริปต์และผู้พัฒนาลงล็อกที่ถูกต้อง
SetText("NameHub", "<font color='#3b82f6'>!!</font> anime mod menu <font color='#3b82f6'>!!</font>")
SetText("Credit", "<font color='#ef4444'>!!</font> Ui kaitun by i.m_dog <font color='#ef4444'>!!</font>")

-- ตัวแปรเก็บสะสมค่าเบื้องหลังสำหรับรวบรวมประกอบลงช่อง "Currencies"
local currentStatus = "Farm"
local currentLevel = 0
local currentBeli = 0
local currentFragments = 0
local currentRace = "None"
local currentRaceVersion = "V1"
local currentFPS = 0
...
local function DispTime(seconds)
...
-- ฟังก์ชันที่ทำการวาดค่าชุดข้อมูลลง UI ทุกครั้งเมื่อมีสิ่งใดเปลี่ยนแปลง
local function refreshDisplay()
    pcall(function()
        -- 1. บรรทัดข้อมูลสถานะหลัก
        SetText("Task1", "MainStatus : <font color='#a855f7'>" .. currentStatus .. "</font>")
        
        -- 2. บรรทัดเก็บรายละเอียดผลปีศาจ
        local dfValue = player.Data.DevilFruit.Value
        local dfText = (dfValue == "" or dfValue == "None") and "None" or dfValue
        SetText("Task2", "Devil Fruit : <font color='#f43f5e'>" .. dfText .. "</font>")

        -- 3. บรรทัดจัดเก็บข้อมูลเงิน เลเวล เศษแก้ว และเผ่า (V1-V4)
        local formattedBeli = ""
        if currentBeli >= 1e9 then formattedBeli = string.format("%.2fB", currentBeli/1e9)
        elseif currentBeli >= 1e6 then formattedBeli = string.format("%.1fM", currentBeli/1e6)
        elseif currentBeli >= 1e3 then formattedBeli = string.format("%.1fK", currentBeli/1e3)
        else formattedBeli = tostring(currentBeli) end

        SetText("Currencies", "<font color='#a855f7'>Fragments</font>: " .. currentFragments .. "  |  <font color='#facc15'>Beli</font>: " .. formattedBeli .. "  |  <font color='#22c55e'>Lv</font>: " .. currentLevel .. "  |  <font color='#3b82f6'>Race</font>: " .. currentRace .. " (" .. currentRaceVersion .. ")")
        
        -- 4. บรรทัดค่าประสิทธิภาพเน็ตเวิร์คและเฟรมเรต
        SetText("Melees", "FPS: <font color='#22c55e'>" .. currentFPS .. "</font>  |  Ping: <font color='#ec4899'>" .. currentPing .. "</font>")
    end)
end
...
local function updateLevel()
    if player.Data:FindFirstChild("Level") then
        currentLevel = player.Data.Level.Value
        refreshDisplay()
    end
end
...
local function updateBeli()
    if player.Data:FindFirstChild("Beli") then
        currentBeli = player.Data.Beli.Value
        refreshDisplay()
    end
end
...
local function updateTime()
    ...
    SetText("DebugLine", "Game Time: " .. formatTime(gameTime) .. " (" .. DispTime(gameTime) .. ")")
end
...
-- ตัวจัดการลูปคำนวณเวลา, FPS และค่าปิง
...
task.spawn(function()
    while task.wait(1) do
        pcall(function()
            currentPing = (pcall(function() return Stats.Network.ServerStatsItem["Data Ping"]:GetValueString() end) and Stats.Network.ServerStatsItem["Data Ping"]:GetValueString() or "N/A")
            updateTime()
            updateRace()
        end)
    end
...
L_1_[45]["Status"] = function(newStatus)
    currentStatus = tostring(newStatus)
    
    if L_1_[26] then
        L_1_[26]["Text"] = "Status: " .. currentStatus
    end
...
-- สั่งเริ่มดึงและวาดค่าเริ่มต้นในครั้งแรกทันทีที่รัน
pcall(updateLevel)
pcall(updateBeli)
...
print("✅  [System] แก้ไขบั๊กคีย์ข้อความหลุด และปรับหน้าจอรองรับ 7 แถวสมบูรณ์แล้ว!")
...
WindUI:Notify({
    Title = "anime mod menu",
    Content = "Hopping...",
    Duration = 8,
    Icon = "rbxassetid://116828114235759",
})
								L_1_[45]["wt"](.1)
